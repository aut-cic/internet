...
const _acct = Axios.create({
    baseURL: process.env.ACCT_URL || 'http://172.16.4.133:8001',
});
...
async function user_usage({username, ip}, use_cache = true) {
    ...
    try {
        let api_response = await _acct.get('api/v1/userusage', {
            params: {
                username: process.env.ACCT_USERNAME || username,
                ip: process.env.ACCT_IP || ip
            },
            timeout: 5000
        }).then(res => res.data);
    ...
    // Usage history
    const resetDate = new Date(response.resetDate);
    const usageHistory = _.sortBy(response.usagehistory.map(i => {
        const date = new Date(i.created_date);
        return {
            date,
            usageHuman: humanFileSize(i.usage),
            usage: i.usage,
            effective: date >= resetDate
        }
    },['date'])).reverse();
...
const _units = {
    ...
    fa: ['کیلوبایت', 'مگابایت', 'گیگابایت', 'ترابایت', 'PiB', 'EiB', 'ZiB', 'YiB']
}
